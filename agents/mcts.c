...
static inline double fx_uct_score(int n_total, int n_visits, double score)
{
    ...
    return to_Float(
        fx_div(fx16_score, fx16_n_visits) +
        fx_mul(fx_sqrt(to_fx16_int(2)),
               fx_sqrt(fx_div(fx_log(fx16_n_total), fx16_n_visits))));
}
...
static struct node *select_move(struct node *node)
{
    ...
    for (int i = 0; i < N_GRIDS; i++) {
        if (!node->children[i])
            continue;
        double score = fx_uct_score(node->n_visits, node->children[i]->n_visits,
